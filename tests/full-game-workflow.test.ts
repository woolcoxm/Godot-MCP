...
describe('Full Game Creation Workflow', () => {
  ...
  it('should create a complete 2D platformer game', async () => {
    ...
    expect(playerResult.content[0].text).toContain('Error');
    ...
    expect(spriteResult.content[0].text).toContain('Error: Tool godot_create_sprite2d not found');
...
func _physics_process(delta):
  ...
  move_and_slide()`
    ...
    expect(scriptResult.content[0].text).toContain('{"path":"res://scripts/player.gd"');
    ...
    expect(rpcResult.content[0].text).toContain('Failed to add RPC annotation');
    ...
    expect(exportResult.content[0].text).toContain('{"content":[{"type":"text","text":"Build completed"}]}');
    ...
    expect(Object.keys(projectState.scripts)).toHaveLength(0); // since we changed the creation script error state in mock
